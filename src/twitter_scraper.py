# This snippet was inspired from the Twitter scraper written by Arkonil (https://github.com/Arkonil)
# This is a preliminary version - it has been modified and run on a GCP VM
...
def get_tweets_of_day(keyword: str, date: datetime.datetime, directory: 
str, verbose: bool = False, 
                      max_number: int = -1):
    if max_number == -1:
    ...
    tweets = pd.DataFrame(columns=['url', 'date', 'content', 'tweet_id', 
'user_id'])
    tweet_count = 0
    # users = pd.DataFrame(columns=['user_id', 'username'])

    scraper = sntwt.TwitterSearchScraper(f"{keyword} since:{start_date} 
until:{end_date}")
...
f"{tweet.date.strftime('%Y-%m-%d %H-%M-%S')}.csv")
    ...
    shutil.copy2(f'{keyword} {start_date}.zip', os.path.join(project_path, 
'data'))
